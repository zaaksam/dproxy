<template>
    <div>
        <Row type="flex" style="margin-top: 10px; margin-bottom: 15px">
            ...
            ></Input>
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Checkbox v-model="isExpiredBool">只查找过期数据</Checkbox>
            &nbsp;&nbsp;&nbsp;&nbsp;排序字段：
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;
            <Button type="warning" @click="onClear">清理过期</Button>
        </Row>
        ...
        >
            <Form :label-width="100">
                ...
                <Form-item label="过期时间">
                    <Date-picker
                        type="datetime"
                        :value="modal.expiredStr"
                        placement="top"
                        :options="dateOptions"
                        :editable="false"
                        placeholder="放空默认24小时后过期"
                        style="width: 200px"
                        @on-change="onModalDatetimeChange"
                    ></Date-picker>
                </Form-item>
            </Form>
...
export default class MyWhiteList extends Vue {
    ...
    query: Model.WhiteListQuery = {
        sortField: 'created',
        sortDesc: '1'
    }
    isExpiredBool: boolean = false
    ...
    async onLoad() {
        ...
        if (this.query.ip) {
            pms.append("ip", this.query.ip)
        }

        if (this.query.userName) {
            pms.append("userName", this.query.userName)
        }

        if (this.isExpiredBool) {
            pms.append("isExpired", "1")
        }
    ...
    async onClear() {
        let result = await API.delete<any>("/whitelist/clear");
        if (result.code === 10000) {
            this.onLoad();
        } else {
            this.$Message.error({
